...
if (!semver.satisfies(process.version, version)) {
    ...
    console.log(
        `Asset Manager requires Node v${rawVersion} or newer and you're using ${process.version}`
    );
    process.exit(1);
}
...
if (Array.isArray(config.static) && config.static.length) {
    publicFiles = [...publicFiles, ...config.static];
}
...
self.manifest = {
    "version": "${hash}",
    "assets": [${publicFiles.map((file, index) => {
        return '\n\t\t"' + file + '"';
    })}\n\t]
}
`;
